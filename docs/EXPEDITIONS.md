...
When a diver calls **Need pickup**, an aboard crewmate can choose **Set shared pickup course** on the radio banner. The course follows the diver’s live position on the chart, navigation readout and helm compass. The diver receives guidance back to Kestrel. Your plotted expedition route is kept underneath; boarding or losing contact with the diver restores it. To stop early, open the chart and choose **End pickup · resume course**. Plotting a different destination ends the pickup too. A pickup course continues beyond the radio call’s short display time; it guides the crew but does not steer the ship. Activities prioritizes the pickup: raise anchor, take the helm, hold near the diver, and board. Normal expedition duties return after the pickup ends. Completed pickups remain in the shared voyage log, recording elapsed pickup time, who set the course, who was at the helm, and whether the diver climbed aboard or used the safety beacon. The latest 32 returns survive saved-server restarts. Cancelled pickups and lost contact do not create a completed-return entry. `npm run test:pickup-course:browser` checks the two-player radio flow, live swimming, phone chart, cancellation and boarding restoration.

A green pointer on the physical helm compass follows the **shared plotted course**. Steer toward the side it indicates until it meets the compass’s top index. A crewmate can change the destination on the chart while you remain at the helm. The pointer hides within 15 m of the destination or when the course is cleared; the compass card still shows heading. This is a direct bearing, so the captain must watch for obstacles. `npm run test:helm-course:browser` checks two-player plotting, correct screen direction, native steering, mirrored water geometry, phone rendering, clearing and homeward replanning.

Kestrel’s helm now has a physical **DEPTH** display above the wheel. In the first-person helm view (**C**), it shows whole metres from the ship to the seabed directly below, updating as you sail over the shelf and trench. It follows the same terrain reading as the screen sounder; rocks and wrecks are not included. `npm run test:helm-sounder:browser` verifies the rendered instrument at reef and trench depths, matching underwater copies, and desktop/phone visibility.

New research requests keep a voyage summary with their filed or set-aside report: completed dives, combined recorded dive time, deepest recorded depth, safety-beacon returns and strongest seas during the request. These observations appear in the station receipt and shared voyage log, including the downloaded text. The totals survive rolling dive-history limits and saved-server restarts. Dives are counted when they finish; older requests and reports retain their original information without reconstructed measurements.

At harbor, open **Activities → Kestrel’s crew inscription** to set the nameplate above the aft cabin windows. Use up to 48 English letters, numbers, spaces or basic punctuation. The inscription is shared, stays aboard through later expeditions, and survives saved-server restarts. A crewmate’s edit preserves your draft and requires you to load their version before replacing it. `npm run test:inscription:browser` checks two-player editing, conflict handling, rejoining, phone controls and the physical plate in daylight and storm lighting.

During a research voyage, you can plot a detour and then choose **Resume research guidance** on the chart. The requested habitat briefing returns, with guidance to raise anchor, sail, anchor near the site and dive. **Plan research dive** selects the requested habitat; after the survey it becomes **Plan research return** and selects Pelican Station. Partial survey work stays with the crew throughout. After surveying, the notebook offers **Ask dive team to return** while divers remain in the water. They can acknowledge the recall over the shared radio. Return guidance then follows the crew through raising anchor, sailing home, anchoring beside Pelican Station and filing the report.


The crew notebook now includes **Dive team return** beneath the manifest while connected divers are in the water. For each diver it estimates a direct ascent and surface swim to Kestrel's current position at full swim speed, plus expected sea conditions on arrival when an outlook is available. Readings update as the diver or cutter moves; a moving-ship note explains that the destination is changing. Allow extra time for obstacles and boarding. **Bearing [K]** also gives the diver a brief return estimate. Reconnecting pauses the readings, and boarding or disconnecting removes the diver from the briefing. `npm run test:dive-return:browser` checks the live notebook, different-depth forecasts, ship movement and phone layouts; the game tests also exercise the estimated route through actual swim input.

Longer exploration and research voyages now encounter passing weather fronts. After archive delivery, the existing squall clears; the first new front begins building seven minutes later in shared voyage time. Fronts build over two minutes, pass at peak strength for one minute, and ease over three minutes, with long fair intervals between them. Their strength varies reproducibly across the 15-minute cycles. **Chart → Weather & dive plan** and crew weather calls use this same weather clock, so a short dive and a long stay can have different return conditions.

The weather clock survives server restarts and pauses with an empty voyage. Older completed saves begin from their saved sea state when resumed. Accepting research keeps the clock running; accepting salvage carries the current sea state smoothly into that job's weather. These are composed game-weather patterns, also driving the existing clouds, rain, waves and vessel motion. `npm run test:weather-planner:browser` checks planning around a front, crew reports and rendered rain and clearing.

After delivering the first archive, open **Activities → Pelican research requests** at harbor. Choose an unsurveyed habitat and **Accept research request** to share its course with the crew. Sail there, anchor, dive and hold **X** at the habitat signal. Existing partial survey work counts, and nearby divers can help. Completing the requested survey changes its course to homeward guidance; a separately plotted detour stays in place.

Return to Pelican Station with every connected diver aboard, slow and anchor, then choose **File research report** in Activities. The notebook shows the station's receipt and survey team, and the shared voyage log keeps the request's acceptance and outcome. Requests, partial work and the latest 32 outcomes survive server restarts. You may **Set aside request at harbor** to keep your discoveries and choose different work. File or set aside the current request before accepting another research or salvage job. Research follows the current sea conditions; it does not trigger a new salvage squall. `npm run test:research:browser` checks shared acceptance, a late join, native surveying and boarding, filing and phone layouts. The game tests include a complete research journey using public sailing and swimming rules.

**Tools [I]** now includes a live seabed reading. Aboard Kestrel it measures the water depth beneath the ship; while diving it shows your height above the seabed. **Bearing [K]** includes the same reading briefly, keeping the normal view clear. Measurements follow the actual terrain at the ship or diver, independently of the plotted destination, and pause while reconnecting. They do not measure clearance from individual rocks, wrecks or equipment. `npm run test:depth-sounder:browser` checks shallow and deep readings, native controls, diver ascent and phone layouts.

After identifying an animal with **Observe [O]**, open **Field notes & sharing** and choose **Share sighting**. The first report of each animal group is kept with this crew's voyage, including the observer's name, expedition and position. Everyone's **Field journal [J]** shows shared reports, including while it is already open. Personal observations and photographs remain in their original browser. Reports survive rejoining, server restarts and later salvage expeditions.

The shared ship's log includes these wildlife reports. Choose **Save location to chart** on a sighting entry to keep a named place, add crew notes or plot a return course. This records the observer's location when sharing, rather than tracking the moving animal. Identification is checked in the observing browser; the server records it as a crew report and supplies the observer, voyage time and position. `npm run test:crew-sightings:browser` verifies real animal identification, explicit sharing, a live peer journal, phone layout and chart navigation.

Exploring a wreck now leaves a shared memory, even when it is not your assigned salvage site. A connected diver who reaches within 12 m of the wreck's seabed archive location, below 3 m depth, records the first visit and nearby crewmates in the voyage log. The chart keeps a named **Explored wreck** destination for return visits. Plotting it changes only the crew course; salvage assignments still come from Pelican Station. These records survive rejoining, server restarts and later expeditions. `npm run test:wreck-discoveries:browser` checks shared discovery, chart navigation, the searchable log and phone layouts using a diver positioned at the western wreck.

At Pelican Station, open **Activities → Kestrel’s hull paint** to choose harbor teal, ocean blue or oxide red. Apply paint while aboard the anchored, slow cutter within 24 m of the station. The choice is shared with every crewmate and survives rejoining, server restarts and later salvage jobs. It changes the hull's finish without changing handling or equipment. `npm run test:hull-paint:browser` checks two-client selection, isolated materials, matching water copies and phone controls.

Use **Search the voyage log** to find entries by crewmate, place, expedition number, time or remembered words. Search matches all entered words regardless of capitalization. It filters only your notebook view; **Save voyage log** still exports the full history. Closing the notebook keeps your search for the current visit, and **Clear search** restores every entry.

**Chart → Plan a course home** selects Pelican Station at any stage of an expedition. Plotting shares the homeward route with crew aboard while directing divers back to Kestrel's boarding ladder. The chart reports connected divers still in the water, and its passage estimate shows sailing time without a dive or exploration stay. Returning home does not erase the current mission or discoveries.

During a dive, the notebook's **Crew manifest** shows the diver's current depth, elapsed dive time and deepest measured point. These readings follow shared voyage time and clear from the current-duty row when the diver boards; the completed record remains in the voyage log. Compact navigation labels continue to show current depth only.

Returning aboard records a dive in the shared voyage log: diver name, elapsed voyage time in the water, maximum measured depth, expedition number, and whether the diver climbed aboard or used the safety beacon. The latest 64 completed dives are retained and included in log exports. Active dive timing and finished records survive server restarts. Time continues during a disconnect if other crew keep the voyage running; an empty voyage pauses. Older dives without a recorded start time do not generate an invented history.

Wreck art checks can target each salvage site: `node tools/wreck-browser.mjs review west` or `node tools/wreck-browser.mjs review east`. They capture approach, interior, wide and mooring views in controlled lighting, verify water-copy counts and mooring endpoints, and report geometry and GPU timings. Loose seabed plating uses subdivided faces to follow the sand between its corners.

The salvage planner now includes **Recovery & return**: time to get the crew aboard, remaining winch time, the homeward sail, and estimated arrival at Pelican Station. Estimates update during lifting and after recovery. They assume divers board before an uninterrupted lift and a direct full-speed sail; actual crews can overlap diving and lifting. Allow additional time for positioning, anchoring and handling the ship. Return weather uses the current weather phase and remains unknown when no timed outlook exists.

New crew log entries record the writer's location at submission: the cutter's surface position while aboard, or the diver's position and depth underwater. **Save location to chart** creates a shared saved place with the entry's text as its note. You can rename it and plot a return course in the voyage chart. Repeated saves reuse the same place, and older entries without coordinates remain readable without inventing a location.

Open **Activities → Voyage log → Add a crew entry** to write a shared memory or decision. Entries retain the writer's name, voyage time and expedition number, survive later salvage jobs and server restarts, and appear in **Save voyage log** exports. Closing the notebook keeps an unfinished draft for the current visit. The log holds up to 64 crew entries of 400 characters; existing entries are never discarded to make room. Entries are permanent additions, so record corrections as another entry.

Each diver can independently acknowledge a return-aboard call. The radio lists the responders alongside the live count still in the water. Crew already aboard cannot answer on behalf of the dive team; disconnected responders are removed from the call.

While connected divers are in the water, crew aboard can use **Radio → Return aboard, please**. The call shows the live number of divers still out, accepts the normal “On it” acknowledgement, and clears when all connected divers return (or when the call expires). Acknowledging does not move a diver or mean that everyone is aboard.

Open **Chart → Plan current salvage** to inspect the assigned archive's actual depth and sailing distance in the weather and dive planner. The destination follows each newly accepted job. Plotting it shares a course with the crew; its instructions cover cable attachment and recovery. It does not count as a habitat survey. Attaching the cable automatically completes the assigned salvage course: divers receive the moving boarding target, and deck crew receive recovery guidance. Independently plotted habitat or saved-place courses remain selected. The separate **Recovery & return** estimate includes winch work and the homeward sail.

The voyage log identifies each later salvage expedition by number and wreck name, records its acceptance time, and includes the duration at delivery. These entries stay attributed to the same expedition after accepting another job and appear in the downloaded text log. Older saved histories without acceptance times remain readable without inventing a start time.

After delivery, open **Activities → Another salvage expedition** to choose another of three wreck sites. Kestrel must be anchored within 24 m of Pelican Station, moving below 1.5 m/s, with every connected diver aboard. Any connected crewmate can accept; the first accepted job becomes the shared expedition. The western and eastern archives occupy separate physical wrecks, with their own seabed depths, buoy positions and recovery distances.

Accepting clears the active cargo, course and mission milestones while preserving crew identities, surveys, saved places and field notes. Existing weather eases continuously as the next voyage begins. Each later delivery reports that expedition's duration. The voyage log retains the latest 20 previous salvage records alongside the current expedition; the accepted job and history survive server restarts.

`npm run test:salvage-voyages:browser` uses harbor delivery fixtures and native notebook controls to check both new job selections, repeated completion receipts, desktop/phone layout, rendered buoy/cargo relocation and rejoining. The game logic suite sails, dives, lifts and returns at both additional sites through normal movement/actions; persistence tests restart the server after accepting a second job.
...
**Plan around the weather.** The voyage chart's **Weather & dive plan** section shows the current sea, the active squall's two-minute outlook, and estimates for the selected destination. Choose 30 seconds, one minute, three minutes or five minutes exploring to compare likely surface conditions when you arrive and resurface. Plans start from Kestrel's current position and use full-speed sailing and direct swimming in the game's compressed time; allow extra time for handling the ship, boarding and equipment. These are optimistic estimates, not a timer or a restriction on diving. A surface bookmark uses a visit estimate instead of a descent.

The outlook uses the same storm curve as the server, including clearing after archive delivery. Before the squall starts, it reports that no timed outlook is available. A prediction assumes the current mission phase continues. **Crew radio [Z] → Share weather outlook** sends the current outlook to connected crewmates, who can acknowledge it. The message keeps conditions at call time and expires with the normal crew calls. `npm run test:weather-planner:browser` verifies destination and duration choices, missing forecasts, clearing weather, phone layout and an acknowledged radio report; logic tests compare a prediction with two minutes of real server simulation.

**Keep places your crew finds.** Mark the water or seabed with **G**, then open **Chart [N] → Keep a crew mark**. Choose an active mark, give it a name and select **Save place to chart** before its 18-second signal fades. Any crewmate can save a shared mark. The saved location retains its actual position and depth, appears in the voyage log, and joins the chart's destination list. Plot it to sail back or guide a diver to the marked depth. Saved places are personal crew destinations; they do not create new habitat survey tasks or change the survey total.

Each voyage keeps up to 24 named places across rejoining and saved-server restarts. To make room, choose a saved place and **Remove saved place**; removing the current destination restores mission guidance for everyone. This also removes that place's entry from the current voyage log. `npm run test:saved-places:browser` checks two players sharing a place, naming, automatic selection, phone layout, signal expiry, return courses, rejoining and removal. Logic and persistence checks cover untrusted coordinates, expired marks, the limit and disk restoration.

Choose a saved place and open **Edit crew notes** to rename it or record up to 400 characters of observations, approaches or reasons to return. Notes appear with the destination and in the voyage log, including its downloadable text. Editing preserves the location, original saved record and any course already plotted there. The editor identifies the latest contributor, and notes survive saved-voyage restarts.

Drafts stay in this visit while switching chart destinations or closing the chart. If a crewmate edits the same place, your draft stays visible and Save is disabled; the latest shared note remains visible above the editor. **Reload crew notes** explicitly replaces your draft with that version so you can combine observations and save again. Reloading the browser discards unsaved drafts. `npm run test:place-notes:browser` checks native editing, draft retention, conflicting crew edits, explicit reload, course preservation, phone layout and rejoining; server tests reject stale edits and verify disk persistence.

Open **Activities → Voyage log** to read the crew's shared history. Leaving Pelican Station, locating the wreck, attaching the cable and securing the archive are recorded once, with the crew or operator responsible. Completed habitat surveys add their depth and contributors; archive delivery includes its original receipt. Entries use elapsed voyage time and survive saved-voyage restarts. **Save voyage log** downloads a text copy to keep outside the game. Older saves retain their existing survey and delivery history; missing past milestones are not reconstructed. The log stays inside the notebook and adds no floating notices.

`npm run test:voyage-log:browser` checks live notebook updates, the empty state, input isolation, desktop/phone layouts, text download and rejoining. Logic tests cover milestone attribution and deduplication; persistence tests verify a real disk checkpoint and server restart.

Kestrel also keeps a physical **ship’s logbook** in a shallow rack on the starboard side of the aft cabin wall, opposite the deck radio. Walk up and press **F** or **Read ship’s log** to open the voyage history directly. The weatherproof ledger, ruled pages, retaining clip and pencil use the ship's procedural materials and water rendering. Reading leaves your station and the captain's controls unchanged. Escape closes the notebook; keyboard focus returns to the nearby logbook control. `npm run test:deck-logbook:browser` checks walking, keyboard/pointer access, live history, modal input isolation, desktop/phone layouts, the underwater copy and daylight/storm rendering.

The **voyage chart [N]** also offers fifteen habitat surveys for this crew, before or after the archive recovery. Plot a destination, sail there, and dive to its underwater signal—or explore freely and survey a habitat you encounter. Within 18 m of a survey site at its actual depth, its name and survey controls appear even without a plotted course. **Hold X** or the **Hold to survey** button. A survey takes eight seconds of combined diver effort: two scanning divers at the same site finish in four seconds. Separate dive teams can survey different habitats simultaneously; each site's progress and contributors remain separate. Changing the ship's course does not interrupt divers working at their actual site. You can swim and look around while scanning; leaving range or releasing the control pauses your contribution. Completed work appears on the crew chart and in the voyage log.
